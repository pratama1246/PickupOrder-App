    ...
    <section class="px-3 sm:px-10 md:px-16 lg:px-24 mb-8">
        <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
            <div class="flex items-center gap-3 w-full sm:w-auto">
                <select x-model="selectedStatus" class="select select-bordered select-md rounded-full border-base-content/40 w-full sm:w-auto min-w-48 focus:outline-none text-sm sm:text-base">
            ...
            <form action="" method="GET" @submit.prevent class="w-full sm:max-w-md relative">
                <label class="input input-bordered flex items-center w-full shadow-sm rounded-3xl border-base-content/40 focus-within:border-base-content input-md pr-12">
                    <input type="search" x-model="searchQuery" class="grow text-sm sm:text-base pl-2" placeholder="Cari riwayat pesanan..." />
                </label>
            ...
            <div class="max-w-4xl space-y-6">
                ...
                @if ($pendingOnlineGroups->isNotEmpty())
                    @foreach ($pendingOnlineGroups as $paymentCode => $group)
                        ...
                        @endphp
                        <div x-show="(selectedStatus === 'Semua Status' || '{{ $statusStr }}' === selectedStatus) && ('{{ addslashes($searchStr) }}'.includes(searchQuery.toLowerCase()) || searchQuery === '')" x-transition>
