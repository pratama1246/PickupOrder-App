...
<div class="bg-vanilla-custard-50 border border-base-content/30 rounded-3xl p-6 sm:p-8 shadow-sm">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pt-2">
        ...
        <div class="flex-1 max-w-sm w-full">
            ...
            {{-- Peringatan: pesanan Midtrans yang masih pending tidak boleh diproses --}}
                ...
                <div class="bg-amber-50 border border-amber-300 rounded-xl p-4 mb-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-amber-700 text-xs font-medium mt-0.5">Pesanan ini belum dapat diproses. Silakan tunggu konfirmasi pembayaran dari mahasiswa.</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                @if (in_array($order->status, ['menunggu', 'dimasak', 'siap_diambil']))
                    @if ($order->payment_method === 'midtrans' && $order->payment_status === 'pending')
                        {{-- Nonaktifkan tombol jika pembayaran online belum dikonfirmasi --}}
                        <button type="button" disabled class="btn bg-base-300 text-base-content/40 border-none w-full rounded-xl font-bold shadow-sm cursor-not-allowed flex-1">
                            Menunggu Pembayaran
                        </button>
                    @else
