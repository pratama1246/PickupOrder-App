...
--}}
<div class="navbar bg-fern-50/90 backdrop-blur-md border-b border-fern-100/80 px-6 sticky top-0 z-50 h-20 transition-all">
    ...
    <div class="navbar-center hidden lg:flex">
        <ul class="menu menu-horizontal px-1 gap-2 text-base font-semibold">
            <li>
                <a href="/"
                    class="rounded-xl px-4 py-2 transition-all {{ request()->is('/') ? 'bg-fern-700 text-white font-bold shadow-xs' : 'text-fern-900/75 hover:text-fern-900 hover:bg-fern-100/70 font-medium' }}">
                    Beranda
                </a>
            </li>
            <li>
                <a href="{{ route('canteen.index') }}"
                    class="rounded-xl px-4 py-2 transition-all {{ request()->is('browse*') || request()->is('canteen*') ? 'bg-fern-700 text-white font-bold shadow-xs' : 'text-fern-900/75 hover:text-fern-900 hover:bg-fern-100/70 font-medium' }}">
                    Pesan
                </a>
            </li>
            ...
            <li>
                <a href="{{ route('order.index') }}"
                    class="rounded-xl px-4 py-2 transition-all {{ request()->is('history*') ? 'bg-fern-700 text-white font-bold shadow-xs' : 'text-fern-900/75 hover:text-fern-900 hover:bg-fern-100/70 font-medium' }}">
                    Riwayat
                </a>
            </li>
            @endif
            <li>
                <a href="{{ route('about') }}"
                    class="rounded-xl px-4 py-2 transition-all {{ request()->routeIs('about') ? 'bg-fern-700 text-white font-bold shadow-xs' : 'text-fern-900/75 hover:text-fern-900 hover:bg-fern-100/70 font-medium' }}">
                    Tentang Kami
                </a>
            </li>
        </ul>
    ...
    <div class="navbar-end gap-2">
        ...
        @endphp
        <a href="{{ route('cart.index') }}" class="btn btn-ghost btn-circle text-fern-900 hover:bg-fern-100/70 transition-colors" id="navbar-cart-btn" aria-label="Keranjang Belanja">
            <div class="indicator">
                ...
                @if ($showCartBadge && $cartCount > 0)
                    <span id="navbar-cart-count" class="badge badge-sm indicator-item bg-fern-700 text-white border-0 font-bold shadow-xs">
                        {{ $cartCount }}
                    </span>
                @endif
