...
<div class="bg-base-100 rounded-3xl p-5 shadow-sm border border-base-200 flex justify-between items-center gap-4">
    <div class="flex-1 min-w-0">
        <div>
            <h3 class="text-sm font-medium text-base-content/60">{{ $label }}</h3>
            <p class="text-2xl sm:text-3xl font-bold mt-1 {{ $valueColor }}">{{ $value }}</p>
        </div>
    ...
    @if(isset($icon))
        <div class="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center shrink-0 [&_svg]:w-7 [&_svg]:h-7 sm:[&_svg]:w-8 sm:[&_svg]:h-8 [&_svg]:stroke-[1.75] {{ $iconBg }}">
            {{ $icon }}
        </div>
    @endif
</div>
