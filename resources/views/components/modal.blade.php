...
<dialog id="{{ $id }}" {{ $attributes->merge(['class' => 'modal modal-bottom sm:modal-middle']) }}>
    <div class="modal-box bg-base-100 rounded-t-2xl sm:rounded-2xl border border-base-content/5 p-6 shadow-xl {{ $modalClass }}">
        
        <!-- Tombol Tutup Pojok Atas -->
        @if($showClose)
            <button onclick="document.getElementById('{{ $id }}').close()" type="button" class="btn btn-sm btn-circle btn-ghost absolute right-4 top-4 text-base-content/50 hover:text-base-content" aria-label="Tutup">
