    ...
    <div class="flex flex-col flex-1 card-body p-6 md:p-2">
        ...
        <div class="mt-auto flex justify-end gap-3 flex-wrap">
            @if (isset($buttons))
                {{ $buttons }}
            @else
                <a href="{{ $actionUrl }}" class="btn bg-fern-700 text-white hover:bg-fern-800 border-none shadow-md rounded-xl w-full md:w-fit transition-all duration-200 active:scale-95 hover:shadow-lg font-bold text-sm px-6 py-2 min-h-0 h-auto">
                    {{ $actionText }}
                </a>
            @endif
        </div>
