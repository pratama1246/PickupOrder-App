    ...
    <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mt-6 sm:mt-8">
        ...
        <div class="flex gap-3 w-full sm:w-auto">
            ...
            @if ($order->status == 'menunggu' || $order->status == 'dimasak' || $order->status == 'siap_diambil')
                <a href="{{ route('order.show', $order->id) }}" class="btn bg-fern-700 hover:bg-fern-800 text-white border-none w-full sm:w-auto px-8 py-2 min-h-0 h-auto rounded-md font-bold text-sm text-center">
                    Pantau Antrian
                </a>
            @endif
        </div>
