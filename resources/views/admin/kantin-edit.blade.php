...
@section('content')
    ...
    <div
        ...
        <div class="mb-6 sm:mb-8">
            ...
            <p class="text-base-content/70 text-sm sm:text-lg font-medium">Perbarui informasi pengelola atau status kantin.
            </p>
        </div>
        ...
        <form action="{{ route('admin.kantin.update', $canteen->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <select name="is_open"
                    ...
                    <option value="1" {{ old('is_open', $canteen->is_open ? '1' : '0') == '1' ? 'selected' : '' }}>Buka
                    </option>
                    <option value="0" {{ old('is_open', $canteen->is_open ? '1' : '0') == '0' ? 'selected' : '' }}>
                        Tutup</option>
                </select>
                @error('is_open')
                    <p class="text-error text-xs mt-1">{{ $message }}</p>
