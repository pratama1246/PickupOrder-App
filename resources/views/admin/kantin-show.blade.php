...
<div class="max-w-8xl mx-auto space-y-6 pb-10 lg:pb-0" x-data="{ activeTab: 'menu' }" x-cloak>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center justify-between md:justify-start gap-4 w-full md:w-auto">
            <div>
                ...
                <p class="text-base-content/70 text-sm sm:text-base font-medium">Informasi lengkap, menu, dan statistik transaksi kantin.</p>
            </div>
            
            <!-- Action Buttons (Mobile only, Icon-only) -->
            <div class="flex md:hidden items-center gap-2 shrink-0">
                <button type="button" onclick="document.getElementById('delete_canteen_modal_{{ $canteen->id }}').showModal()" class="btn bg-red-600 hover:bg-red-700 text-white border-none shadow-sm rounded-md transition-colors p-2.5 h-auto min-h-0 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
                </button>
                <a href="{{ route('admin.kantin.edit', $canteen->id) }}" class="btn bg-fern-700 hover:bg-fern-800 text-white border-none shadow-sm rounded-md transition-colors p-2.5 h-auto min-h-0 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="hidden md:flex items-center gap-2 shrink-0">
            <button type="button" onclick="document.getElementById('delete_canteen_modal_{{ $canteen->id }}').showModal()" class="btn bg-red-600 hover:bg-red-700 text-white border-none shadow-sm rounded-xl transition-colors text-sm font-bold px-5">
                Hapus
            </button>
            <a href="{{ route('admin.kantin.edit', $canteen->id) }}" class="btn bg-fern-700 hover:bg-fern-800 text-white border-none shadow-sm rounded-xl transition-colors text-sm font-bold px-5">
                Edit
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-base-200 overflow-hidden flex flex-col md:flex-row">
        ...
        <div class="w-full md:w-1/3 h-48 md:h-auto relative bg-base-200">
            ...
            <div class="absolute bottom-4 left-4 md:hidden">
                ...
                @else
                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-rose-100 text-rose-800 shadow-sm border border-rose-200">
        ...
        <div class="p-6 md:p-8 flex-1 flex flex-col justify-center">
            <div class="flex items-start justify-between mb-2">
                <div>
                    ...
                    <p class="text-base-content/70 text-sm sm:text-base font-medium max-w-xl">{{ $canteen->description ?: 'Tidak ada deskripsi.' }}</p>
                </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="w-12 h-12 rounded-full bg-base-200 ring-2 ring-fern-200 flex items-center justify-center shrink-0 overflow-hidden">
                    <img src="{{ asset('storage/' . $canteen->owner->avatar) }}" alt="{{ optional($canteen->owner)->name }}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" />
                    <div class="w-full h-full bg-fern-100 text-fern-700 items-center justify-center hidden">
                        <span class="text-xl font-bold uppercase">{{ substr(optional($canteen->owner)->name ?? 'V', 0, 1) }}</span>
                    </div>
                </div>
                @else
                <div class="w-12 h-12 rounded-full bg-fern-100 text-fern-700 ring-2 ring-fern-200 flex items-center justify-center shrink-0">
                    <span class="text-xl font-bold uppercase">{{ substr(optional($canteen->owner)->name ?? 'V', 0, 1) }}</span>
                </div>
                @endif
                <div>
                    ...
                    <a href="mailto:{{ optional($canteen->owner)->email }}" class="text-sm text-fern-600 hover:text-fern-700 font-medium transition-colors">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-base-200 overflow-hidden mt-8">
        <div class="flex items-center gap-6 px-6 border-b border-base-200 overflow-x-auto scrollbar-hide bg-base-100/50">
            <button type="button" @click="activeTab = 'menu'" :class="activeTab === 'menu' ? 'border-fern-700 text-fern-700' : 'border-transparent text-base-content/60 hover:text-base-content'" class="py-4 font-bold text-sm sm:text-base border-b-2 transition-colors whitespace-nowrap">
                Daftar Menu
            </button>
            <button type="button" @click="activeTab = 'order'" :class="activeTab === 'order' ? 'border-fern-700 text-fern-700' : 'border-transparent text-base-content/60 hover:text-base-content'" class="py-4 font-bold text-sm sm:text-base border-b-2 transition-colors whitespace-nowrap">
                Transaksi Terbaru
            </button>
        </div>
        ...
        <div class="p-0">
            <!-- TAB: MENU -->
            <div x-show="activeTab === 'menu'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                                ...
                                <tr class="hover:bg-base-200/30 transition-colors border-b border-base-content/5">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-4">
                                            <div class="w-12 h-12 rounded-xl bg-base-200 overflow-hidden shrink-0 border border-base-content/10">
                                                <img src="{{ $menu->image ? asset('storage/' . $menu->image) : asset('assets/food/es teh.jpg') }}" alt="{{ $menu->name }}" class="w-full h-full object-cover" onerror="this.src='{{ asset('assets/food/es teh.jpg') }}'"/>
                                            </div>
                                            <div>
                                                <div class="font-bold text-base-content text-sm sm:text-base">{{ $menu->name }}</div>
                                                <div class="text-xs text-base-content/60 line-clamp-1 max-w-[200px]">{{ $menu->description ?: '-' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if($menu->is_available && $menu->stock > 0)
                                            <span class="inline-flex px-2.5 py-1 bg-emerald-100 text-emerald-800 rounded-md text-xs font-bold border border-emerald-200 shadow-sm">Tersedia</span>
                                        @else
                                            <span class="inline-flex px-2.5 py-1 bg-rose-100 text-rose-800 rounded-md text-xs font-bold border border-rose-200 shadow-sm">Habis</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
            ...
            <!-- TAB: ORDERS -->
            <div x-show="activeTab === 'order'" style="display: none;" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                <div class="overflow-x-auto">
                    <table class="table w-full">
                        <thead class="bg-base-200/50 text-xs text-base-content/70">
                            <tr>
                            ...
                            @forelse($orders as $order)
                                <tr class="hover:bg-base-200/30 transition-colors border-b border-base-content/5">
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-base-content/50">{{ $order->created_at->format('d M Y, H:i') }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="font-bold text-sm">{{ optional($order->user)->name ?? 'Tidak diketahui' }}</div>
                                        <div class="text-xs text-base-content/60">Waktu Ambil: {{ $order->pickup_time->format('H:i') }}</div>
                                    </td>
...
<x-modal id="delete_canteen_modal_{{ $canteen->id }}" type="error" title="Hapus Kantin">
    Apakah Anda yakin ingin menghapus kantin <strong>{{ $canteen->name }}</strong>? Seluruh data menu dan pesanan terkait akan ikut terhapus.
