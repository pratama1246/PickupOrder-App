...
@section('content')
    ...
    <div
        ...
        <x-breadcrumb compact :links="[['label' => 'Pengguna', 'url' => route('admin.users.index')], ['label' => 'Tambah Pengguna']]" />
        ...
        --}}
        <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-5" x-data="{ role: '{{ old('role', '') }}' }">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-4" :class="role !== 'vendor' ? 'sm:grid-cols-2' : ''">
                <div x-show="role !== 'vendor'" x-transition class="transition-colors duration-300">
                    ...
                    <input type="text" name="nim" value="{{ old('nim') }}" :disabled="role === 'vendor'"
                        :required="role !== 'vendor'" placeholder="Masukkan NIM atau NIP"
                        class="input input-bordered w-full rounded-xl border-base-content/25 focus:outline-none focus:border-fern-600 text-sm font-medium" />
                    @error('nim')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="Contoh: user@pnc.ac.id"
                        required
                        class="input input-bordered w-full rounded-xl border-base-content/25 focus:outline-none focus:border-fern-600 text-sm font-medium" />
                    @error('email')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="role" x-model="role" required
                        ...
                        <option value="vendor" {{ old('role') == 'vendor' ? 'selected' : '' }}>Vendor (Pemilik Kantin)
                        </option>
                    </select>
                    @error('role')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="is_first_login" required
                        ...
                        <option value="1" {{ old('is_first_login', '1') === '1' ? 'selected' : '' }}>Nonaktif (Belum
                            Aktif)</option>
                    </select>
