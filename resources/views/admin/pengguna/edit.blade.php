...
@section('content')
    ...
    <div
        ...
        <div class="mb-6 sm:mb-8">
            ...
            <p class="text-base-content/70 text-sm sm:text-lg font-medium">Perbarui informasi dasar dan hak akses akun
                pengguna.</p>
        </div>
            ...
            <div class="grid grid-cols-1 gap-4" :class="role !== 'vendor' ? 'sm:grid-cols-2' : ''">
                <div x-show="role !== 'vendor'" x-transition class="transition-colors duration-300">
                    ...
                    <input type="text" name="nim" value="{{ old('nim', $user->nim) }}"
                        :disabled="role === 'vendor'" :required="role !== 'vendor'" placeholder="Masukkan NIM atau NIP"
                        class="input input-bordered w-full rounded-xl border-base-content/25 focus:outline-none focus:border-fern-600 text-sm font-medium" />
                    @error('nim')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $user->email) }}"
                        placeholder="Contoh: user@pnc.ac.id" required
                        class="input input-bordered w-full rounded-xl border-base-content/25 focus:outline-none focus:border-fern-600 text-sm font-medium" />
                    @error('email')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="role" x-model="role" required
                        ...
                        <option value="vendor" {{ old('role', $user->role) == 'vendor' ? 'selected' : '' }}>Vendor (Pemilik
                            Kantin)</option>
                    </select>
                    @error('role')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="is_first_login" required
                        class="select select-bordered w-full rounded-xl border-base-content/25 focus:outline-none focus:border-fern-600 text-sm font-medium">
                        <option value="0"
                            {{ old('is_first_login', $user->is_first_login ? '1' : '0') == '0' ? 'selected' : '' }}>Aktif
                        </option>
                        <option value="1"
                            {{ old('is_first_login', $user->is_first_login ? '1' : '0') == '1' ? 'selected' : '' }}>
                            Nonaktif (Belum Aktif)</option>
                    </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <button type="button"
                            ...
                            <svg x-show="!showPw" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <button type="button"
                            ...
                            <svg x-show="!showConfirmPw" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                            </svg>
                            <svg x-show="showConfirmPw" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5"
