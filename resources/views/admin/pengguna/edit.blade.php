...
@section('title', 'Edit Pengguna - Admin PNC')
...
<div class="max-w-2xl bg-vanilla-custard-50 border border-base-content/20 rounded-3xl p-6 sm:p-8 mb-10 lg:mb-0 shadow-sm">
    ...
    <div class="mb-6 sm:mb-8">
        ...
        <p class="text-base-content/70 text-sm sm:text-lg font-medium">Perbarui informasi dasar dan hak akses akun pengguna.</p>
    </div>
    ...
    <form action="{{ route('admin.pengguna.update', $user->id) }}" method="POST" class="space-y-5" x-data="{ role: '{{ old('role', $user->role) }}' }">
        ...
        <div class="grid grid-cols-1 gap-4" :class="role !== 'vendor' ? 'sm:grid-cols-2' : ''">
            <div x-show="role !== 'vendor'" x-transition class="transition-colors duration-300">
                ...
                <input type="text" name="nim" value="{{ old('nim', $user->nim) }}" :disabled="role === 'vendor'" :required="role !== 'vendor'" placeholder="Masukkan NIM atau NIP"
                       class="input input-bordered w-full rounded-xl border-base-content/25 focus:outline-none focus:border-fern-600 text-sm font-medium" />
                @error('nim')
                    <p class="text-error text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email', $user->email) }}" placeholder="Contoh: user@pnc.ac.id" required
                       class="input input-bordered w-full rounded-xl border-base-content/25 focus:outline-none focus:border-fern-600 text-sm font-medium" />
                @error('email')
                    <p class="text-error text-xs mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <select name="role" x-model="role" required
                    ...
                    <option value="vendor" {{ old('role', $user->role) == 'vendor' ? 'selected' : '' }}>Vendor (Pemilik Kantin)</option>
                </select>
                @error('role')
                    <p class="text-error text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="is_first_login" required
                        class="select select-bordered w-full rounded-xl border-base-content/25 focus:outline-none focus:border-fern-600 text-sm font-medium">
                    <option value="0" {{ old('is_first_login', $user->is_first_login ? '1' : '0') == '0' ? 'selected' : '' }}>Aktif</option>
                    <option value="1" {{ old('is_first_login', $user->is_first_login ? '1' : '0') == '1' ? 'selected' : '' }}>Nonaktif (Belum Aktif)</option>
                </select>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4" x-data="{ showPw: false, showConfirmPw: false }">
            <div>
                ...
                <div class="relative">
                    <input :type="showPw ? 'text' : 'password'" name="password" placeholder="Biarkan kosong jika tidak diubah"
                ...
                <p class="text-xs text-base-content/50 mt-1">Minimal 8 karakter jika ingin diubah</p>
                @error('password')
                    <p class="text-error text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    <input :type="showConfirmPw ? 'text' : 'password'" name="password_confirmation" placeholder="Ulangi password baru"
                           class="input input-bordered w-full rounded-xl border-base-content/25 focus:outline-none focus:border-fern-600 text-sm font-medium pr-11" />
                    <button type="button"
                            class="absolute right-3 top-1/2 -translate-y-1/2 text-base-content/40 hover:text-base-content/70 transition-colors"
