    ...
    @if (!App\Helpers\OrderHelper::isOrderTimeActive())
        <div class="bg-amber-500 text-white text-center text-xs sm:text-sm py-2 px-4 shadow-inner flex items-center justify-center gap-2 sticky top-20 z-40">
            ...
            <span class="font-normal">
                <span class="font-semibold">Info Layanan:</span> Pemesanan online tutup (Buka hari {{ App\Helpers\OrderHelper::getActiveDaysFormatted() }} pukul {{ config('app.order_hours.start') }} - {{ config('app.order_hours.end') }} WIB). Anda tetap dapat membeli secara offline langsung di kantin.
            </span>
        </div>
    @endif
